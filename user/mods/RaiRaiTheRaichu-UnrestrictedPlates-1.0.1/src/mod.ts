...
class UnrestrictedPlates implements IPostDBLoadMod, IPreAkiLoadMod
{
    private config;
    ...
    ApplyConfigValues(itemID: string, armorType: string, armorSlot: string, container: DependencyContainer): void 
    {
        const ItemDB = container.resolve<DatabaseServer>("DatabaseServer").getTables().templates.items;
    ...
    LocalizeStrings(langKey: string, armorInfo: string[], container: DependencyContainer): string[]
    {
        const locales = Object.values(container.resolve<DatabaseServer>("DatabaseServer").getTables().locales.global);
